...
const GenerateReport = () => {
  ...
  const fetchAttendanceData = async () => {
    ...
    for (const employeeDoc of employeesSnap.docs) {
      ...
      const attendanceRef = collection(db, "employees", employeeDoc.id, "attendance");
      ...
      attendanceSnap.forEach((attendanceDoc) => {
        ...
        if (attendanceDateObj >= startDate && attendanceDateObj <= currentDate) {
  ...
  const handleGenerateReport = async () => {
    ...
    // Date range
    doc.setFontSize(12);
    doc.text(`Date Range: ${format(startOfMonth(new Date()), "dd-MM-yyyy")} to ${format(new Date(), "dd-MM-yyyy")}`, doc.internal.pageSize.getWidth() / 2, 30, { align: "center" });

    // Table setup
    const headers = ["S.No", "Name", "Email", "Days Present", "Days Absent", "Days Late"];
    ...
    headers.forEach((header, index) => {
      ...
      doc.text(header, xPos + columnWidths[index] / 2, headerYPosition, { align: "center" });
    ...
    reportData.forEach((employee, index) => {
      ...
      rowValues.forEach((value, colIndex) => {
        const xPos = startX + columnWidths.slice(0, colIndex).reduce((a, b) => a + b, 0);
        doc.text(value, xPos + columnWidths[colIndex] / 2, yPosition, { align: "center" });
